...
class OrderController extends Controller
{
    ...
    public function newOrders(Request $request)
    {
        ...
        if ($request->order_type == 'new') {
            ...
            $orders = Order::where('order_status', 1)->whereDate('created_at', $request->get_date)->get();
            $ordersall = 0;
        } elseif ($request->order_type == 'canceled') {
            $orders = Order::where('order_status', 4)->whereDate('created_at', $request->get_date)->get();
            $ordersall = 0;
        } else {
            $orders = [];
            $ordersall = 0;
